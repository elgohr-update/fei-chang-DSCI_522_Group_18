...
We chose to use a simple classification model using the Logistic regression algorithm. To find the model that best predicted whether an individual earns >50k or <=50k, we performed 30-fold cross validation using Cohen's Kappa as our metric of model prediction performance to select K (number of nearest neighbours). We observed that the optimal K was `r model$bestTune$k`.
